class SpicesController < ApplicationController
  before_action :set_spice, only: [:show, :edit, :update, :destroy]
   ...
   def find
    @spices = Spice.where( "spicename = ? OR supplier = ? OR reorder = ?", params[ :search_string ], params[ :search_string], params[ :search_string] )
  end
  ...
  private
    ...
    def spice_params
      params.require(:spice).permit(:spicename, :quantity, :scale, :supplier, :reorder)
    end
end
